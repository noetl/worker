...
mod tests {
    ...
    fn runtime_hook_env_off_disarms_but_primary_keeps_verifying() {
        ...
        assert!(
            runtime_hook_env(&worker_env("/tmp/vec-hook.jsonl", "off")).is_none(),
        ...
        assert!(
            runtime_hook_env(&worker_env("/tmp/vec-hook.jsonl", "primary")).is_some(),
            "`primary` must keep mirroring — it must not silently disarm verification"
        );

        // Control: `shadow` is unchanged by this fix.
        assert!(
            runtime_hook_env(&worker_env("/tmp/vec-hook.jsonl", "shadow")).is_some(),
