...
pub fn runtime_hook_env(env: &EnvMap) -> Option<EnvMap> {
    ...
    match ObjectMode::from_env(env) {
        ObjectMode::Off => return None,
        ObjectMode::Primary => super::warn_primary_not_wired("object"),
        ObjectMode::Shadow => {}
    }
...
mod tests {
    ...
    fn runtime_hook_env_off_disarms_but_primary_keeps_verifying() {
        ...
        assert!(
            runtime_hook_env(&worker_env("/tmp/obj-hook.jsonl", "off")).is_none(),
        ...
        assert!(
            runtime_hook_env(&worker_env("/tmp/obj-hook.jsonl", "primary")).is_some(),
            "`primary` must keep mirroring — it must not silently disarm verification"
        );

        // Control: `shadow` is unchanged by this fix.
        assert!(
            runtime_hook_env(&worker_env("/tmp/obj-hook.jsonl", "shadow")).is_some(),
